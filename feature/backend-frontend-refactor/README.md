...
| MongoDB provisioning and data migration | In progress | Host-run replica-set orchestration, lease-safe immutable migrations, validator evolution, and required real-Mongo CI concurrency/upgrade tests are implemented; managed hosting, backup, and recovery remain |
| Newsletter extraction | In progress | Consent evidence, privacy-safe idempotency, rate limiting, backend ownership, frontend cutover, concurrency tests, and real browser/persistence journey verification are implemented; policy and operational verification remain |

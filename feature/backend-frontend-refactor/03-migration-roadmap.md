...
| Phase 3 — MongoDB/newsletter migration | In progress | Contracts, affirmative consent evidence, privacy-safe idempotency/rate limiting, backend application/repository boundaries, explicit index migration, route, frontend cutover, OpenAPI, and tests are implemented; provisioning, retention/deletion policy, and operational verification remain |

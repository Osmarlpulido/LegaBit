...
| Phase 1 — backend foundation | In progress | API workspace, contracts rename, config validation, MongoDB lifecycle, health/auth/current-user routes, errors, logging, shutdown, tests, reproducible OpenAPI generation, semantic compatibility checks, and CI jobs are implemented; deployment and production routing remain |
| Phase 2 — market data | Complete for the current non-production environment | Contracts, backend route/provider resilience and validation, caching/coalescing, telemetry, nullable-field handling, stale-response protection, tests, frontend cutover, and legacy removal are implemented |
| Phase 3 — MongoDB/newsletter migration | In progress | Contracts, consent evidence, privacy-safe idempotency/rate limiting, concurrency-safe immutable migrations and validator evolution, required real-Mongo CI tests, host-run replica-set orchestration, backend/frontend cutover, and browser/persistence journey verification are implemented; managed provisioning, retention/deletion policy, backup/recovery, and operational verification remain |
...
| Phase 5 — frontend cleanup | In progress | Auth, market, and newsletter traffic use backend-owned routes; import boundaries, direct ESLint, reproducible local-font builds, and Supabase/database dependency removal are implemented; remaining journey tests and compatibility cleanup remain |

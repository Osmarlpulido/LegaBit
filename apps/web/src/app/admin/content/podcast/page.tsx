...
const COLUMNS: ColumnDef<Episode>[] = [
  ...
  { key: "topic", header: "Tema", render: (r) => r.topic },
  { key: "duration", header: "Duración", render: (r) => r.duration },
  {
    key: "status",
    header: "Estado",
...
export default function PodcastAdminPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["admin-podcast", cursor, search],
    queryFn: () => fetch(`/api/admin/content/podcast?${params}`).then((r) => r.json())
  });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Link href="/admin/content/podcast/new" className="rounded-md bg-gray-900 px-3 py-1.5 text-sm font-medium text-white hover:bg-gray-700 transition-colors">
          Nuevo episodio
        </Link>
      </div>
      <DataTable<Episode>
        ...
        actions={(row) => (
          <div className="flex gap-3 justify-end">
            <Link href={`/admin/content/podcast/${row.id}`} className="text-xs text-blue-600 hover:underline">Editar</Link>
            <button onClick={async () => { if (!confirm(`¿Eliminar "${row.title}"?`)) return; await fetch(`/api/admin/content/podcast/${row.id}`, { method: "DELETE" }); qc.invalidateQueries({ queryKey: ["admin-podcast"] }); }} className="text-xs text-red-600 hover:underline">Eliminar</button>
          </div>
        )}
      />
    </div>

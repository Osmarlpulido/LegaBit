...
export const metadata: Metadata = {
  ...
  description: "Episodios de Legabit sobre derecho, tecnología y finanzas disponibles en YouTube y Spotify."
};
    ...
    <MarketingShell>
      <div className="mx-auto max-w-5xl px-6 py-14 sm:py-20">
        ...
        <div className="mb-10 grid gap-4 sm:grid-cols-2">
          <a href={youtubeUrl} target="_blank" rel="noopener noreferrer" className="rounded-xl border border-border bg-background p-5 text-sm font-semibold transition-colors hover:border-legabit-petrol hover:text-legabit-petrol">
            Ver canal en YouTube
            <span className="mt-1 block text-xs font-normal text-muted-foreground">Episodios, directos y clips educativos.</span>
          </a>
          <a href={spotifyUrl} target="_blank" rel="noopener noreferrer" className="rounded-xl border border-border bg-background p-5 text-sm font-semibold transition-colors hover:border-legabit-petrol hover:text-legabit-petrol">
            Escuchar en Spotify
            <span className="mt-1 block text-xs font-normal text-muted-foreground">Audio para seguir los análisis desde cualquier lugar.</span>
          </a>
        </div>
              ...
              <article className="grid gap-5 rounded-2xl border border-border bg-background p-6 shadow-sm sm:grid-cols-[160px_1fr]">
                <div className="flex min-h-32 flex-col justify-between rounded-xl border border-border bg-muted/25 p-4">
                  ...
                  <div className="text-sm text-muted-foreground">
                    <p>{episode.publishedAt ? new Date(episode.publishedAt).toLocaleDateString("es-CO") : "Próximamente"}</p>
                    <p>{episode.duration}</p>
                  </div>
                </div>
                <div className="min-w-0">
                  <h2 className="text-xl font-semibold tracking-tight">{episode.title}</h2>
                  <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{episode.description}</p>

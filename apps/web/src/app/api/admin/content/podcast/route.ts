import { adminCreatePodcastEpisodeSchema, adminListQuerySchema } from "@legabit/api";
...
export async function POST(req: Request): Promise<Response> {
  try {
    ...
    const data = adminCreatePodcastEpisodeSchema.parse(body);

    const episode = await prisma.podcastEpisode.create({ data });

    await logAdminAction({
      actorUserId: admin.prismaUser.id,
      action: "podcastEpisode.create",

...
export async function GET() {
  ...
  let newsletterUses: "supabase" | "none" = "none";
  if (projectUrl && hasServiceKey) newsletterUses = "supabase";

  const payload: Record<string, unknown> = {
    explanation: "El newsletter legacy usa la API REST de Supabase (PostgREST) si hay URL del proyecto y clave de servicio.",
    supabase: {
      projectUrl,
